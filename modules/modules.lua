...
local function LoadConfiguration(Configuration)
	...
	for FlagName, FlagValue in next, Data do
		if RayfieldLibrary.Flags[FlagName] then
			...
			RayfieldLibrary:Notify({Title = "Erro", Content = "Collapse não conseguiu encontrar '"..FlagName.. "'' no script atual"})
		end
	end
end
...
function Hide()
	Debounce = true
	RayfieldLibrary:Notify({Title = "Interface Fechada", Content = "A interface foi fechada, você pode abrir novamente a interface clicando em RightShift", Duration = 7})
	TweenService:Create(Main, TweenInfo.new(0.5, Enum.EasingStyle.Quint), {Size = UDim2.new(0, 470, 0, 280)}):Play()
...
function RayfieldLibrary:CreateWindow(Settings)
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "Bielz Interface Suite"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "by Sirius"
	if Settings.LoadingTitle ~= "Bielz Interface Suite" then
		LoadingFrame.Version.Text = "Bielz UI"
	end
	...
	if Settings.KeySystem then
		...
		if not Passthrough then
			...
			KeyMain.Size = UDim2.new(0, 350, 0, 175)
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						...
						RayfieldLibrary:Notify({Title = "Sistema", Content = "A chave para este script foi salva com sucesso"})
					end
				else
	...
	function Window:CreateTab(Name,Image)
		...
		TabButton.Interact.MouseButton1Click:Connect(function()
			...
			if Elements.UIPageLayout.CurrentPage ~= TabPage then
				TweenService:Create(Elements, TweenInfo.new(1, Enum.EasingStyle.Quint), {Size = UDim2.new(0, 460,0, 250)}):Play()
				Elements.UIPageLayout:JumpTo(TabPage)
				wait(0.2)
				TweenService:Create(Elements, TweenInfo.new(0.8, Enum.EasingStyle.Quint), {Size = UDim2.new(0, 475,0, 250)}):Play()
			end
	...
	if CEnabled then
		pcall(function()
			if isfile(ConfigurationFolder .. "/" .. CFileName .. ConfigurationExtension) then
				...
				RayfieldLibrary:Notify({Title = "Configuração carregada", Content = "O arquivo de configuração para este script foi carregado de uma sessão anterior"})
			end
		end)
	end
end
